...
2. Mutation map (*.mutmap.pdf) : Shows fitness scores of individual mutants.

3. Substitution map (*.mutmap.pdf) : Shows fitness scores of substitutions (mean of individual mutations).
...
6. PDB (*.pdb) : B-factor (temperature/flexibility) factor is written with  position-wise averages of fitness scores.

7. Correlation matrix (*.corr.pdf) : Correlation matrix (Spearman correlation coefficients) between fitness scores (highlighted rows and columns) and molecular features of the mutations.
